...
describe("Product Details Component", () => {
  ...
  // Wait for product details to be loaded onto screen
  const waitForProductLoaded = async () => {
    await waitFor(() => {
  ...
  test("adds main product to cart on click add to cart", async () => {
    ...
    render(<ProductDetails />);
    await waitForProductLoaded();
    userEvent.click(screen.getByTestId("add-to-cart-main"));
  ...
  test("adds similar product to cart on click add to cart", async () => {
    ...
    await waitForProductLoaded();
    userEvent.click(
      screen.getByTestId(`add-to-cart-${similarProducts[0]._id}`)
    );
  ...
  test("adds duplicate similar product to cart if product already in cart", async () => {
    ...
    render(<ProductDetails />);
    userEvent.click(
      await screen.findByTestId(`add-to-cart-${similarProducts[0]._id}`)
    );
