...
test("should create, update, then delete product successfully", async ({
  page,
}) => {
  ...
  const UPDATED_PRODUCT_DETAILS = {
    ...
    description: `${uniqueId} To Kill A Mockingbird book`,
    category: "Book",
    price: "10.99",
    quantity: "100",
    shipping: "Yes",
  ...
  await page
    ...
    .click();
  await expect(getNameField(page)).toHaveValue(UPDATED_PRODUCT_DETAILS.name); // Wait for product details to load before clicking delete
  ...
  await expect(page.getByText("All Products List")).toBeVisible();
  await expect(page.getByText(UPDATED_PRODUCT_DETAILS.name)).not.toBeVisible();
});
